...
pub fn encode_into(
    data: impl AsRef<[u8]>,
    string: &mut std::fmt::Formatter<'_>,
) -> std::fmt::Result {
    let data = data.as_ref();
    for chunk in data.chunks(5) {
        ...
        string.write_str(&ALPHABET[((buf[0] & 0xF8) >> 3) as usize..][..1])?;
        string.write_str(
            &ALPHABET[(((buf[0] & 0x07) << 2) | ((buf[1] & 0xC0) >> 6)) as usize..][..1],
        )?;
        if chunk.len() == 1 {
            break;
        }

        string.write_str(&ALPHABET[((buf[1] & 0x3E) >> 1) as usize..][..1])?;
        string.write_str(
            &ALPHABET[(((buf[1] & 0x01) << 4) | ((buf[2] & 0xF0) >> 4)) as usize..][..1],
        )?;
        if chunk.len() == 2 {
            break;
        }

        string.write_str(&ALPHABET[(((buf[2] & 0x0F) << 1) | (buf[3] >> 7)) as usize..][..1])?;
        if chunk.len() == 3 {
            break;
        }

        string.write_str(&ALPHABET[((buf[3] & 0x7C) >> 2) as usize..][..1])?;
        string.write_str(
            &ALPHABET[(((buf[3] & 0x03) << 3) | ((buf[4] & 0xE0) >> 5)) as usize..][..1],
        )?;
        if chunk.len() == 4 {
            break;
        }

        string.write_str(&ALPHABET[(buf[4] & 0x1F) as usize..][..1])?;
    }
    Ok(())
}
